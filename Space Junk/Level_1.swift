...
class SceneLevel_1:SKScene,SKPhysicsContactDelegate{
    ...
    var gameEndDelegate: GameEndDelegate!
    ...
    @objc func spawnPrize() {
        ...
        if currentPrizeNodes.count < currentPrizeLimit {
            ...
            }else if(prizeSpriteNode?.physicsBody?.categoryBitMask == BitMasks.OrbCategory.rawValue){
                self.addChild(prizeSpriteNode!)
...
//        player.physicsBody?.applyImpulse(CGVector(dx:0,dy:50))
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        //contact between an orb and another orb
        if (itemA.categoryBitMask == BitMasks.OrbCategory.rawValue && itemB.categoryBitMask == BitMasks.OrbCategory.rawValue){
            
            orbToOrbContact(orbAnode: itemA.node!, orbBnode: itemB.node!)
        }
    ...
    func playerLifeContact(playerNode:SKNode,extraLifeNode:SKNode) {
        ...
        currentPrizeNodes.remove(at: currentPrizeNodes.index(of: extraLifeNode as! SKSpriteNode)!)
        updatePlayerState()
    }

    func orbToOrbContact(orbAnode:SKNode,orbBnode:SKNode) {
        explosion(pos: orbBnode.position)
        orbAnode.removeFromParent()
        orbBnode.removeFromParent()
    ...
    func orbHitPlayer(playerNode:SKNode,orbNode:SKNode) {
        ...
        if let playerHealthComponent = player.component(ofType: HealthComponent.self){
            explosion(pos: playerNode.position)
    ...
    func endGame(){
        let scoringComponent = player.component(ofType: ScoreComponent.self)
        gameEndDelegate.GameEnded(finalScore: (scoringComponent?.points)!)
